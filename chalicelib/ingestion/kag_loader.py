...
def get_prepared_kag_batch(base_path, folder_name="Email", limit=5):
    ...
    Returns absolute paths so the Lambda Worker can always find the files.
    """
    ...
    # Get filenames
    files = [f for f in os.listdir(target_dir) if f.lower().endswith(('.jpg', '.jpeg'))][:limit]
    samples = []
    ...
    for i, filename in enumerate(files):
        ...
        samples.append({
            'feedback_id': fid,
            'file_path': abs_file_path,  # <--- Send this to the Bridge
            'folder': folder_name,
            'metadata': {
                'source': 'Kaggle_Tobacco_Set',
                'local_sample_path': abs_file_path
            }
        })
