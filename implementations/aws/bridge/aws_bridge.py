...
class AWSPipelineBridge(IPipelineBridge):
    ...
    def trigger_pipeline(self, data, file=None):
        """Passes the single analysis request directly to the Cloud Pipeline."""
        # This returns the generator (progress, results) for the UI to consume
        return self.pipeline.trigger_pipeline(data)

    def trigger_kag_ingestion(self, base_path, folder_name="Email", limit=5):
        return self.pipeline.trigger_kag_ingestion(base_path, folder_name, limit)

    def trigger_mnist_ingestion(self, digit="0", limit=3):
        return self.pipeline.trigger_mnist_ingestion(digit, limit)
    ...
    def get_record(self, feedback_id):
        """Directly queries DynamoDB for the latest status of a batch item."""
        try:
            table = self.dynamodb.Table('Analysis_Summaries')
            response = table.get_item(Key={'feedback_id': feedback_id})
            return response.get('Item', {})
        except Exception as e:
            print(f"❌ [BRIDGE ERROR] DynamoDB Fetch Failed: {str(e)}")
            return {}